...
if auto_refresh:
    ...
    st.empty() # Placeholder for automatic reload loop in Streamlit
...
def fetch_1min_candles(symbol):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(f"API Error ({symbol}): Status code {response.status_code}")
            return None
    ...
    if len(df) > 0:
        ...
        if latest_spy > 0 and latest_dia > 0:
            if latest_spread >= spread_threshold:
                st.success(f"🚀 **BUY SIGNAL (DIA LAGGING)**: SPY is leading with +{latest_spy:.2f}%, DIA lagging at +{latest_dia:.2f}%. Target mean-reversion move on DIA.")
            elif latest_spread <= -spread_threshold:
                st.success(f"🚀 **BUY SIGNAL (SPY LAGGING)**: DIA is leading with +{latest_dia:.2f}%, SPY lagging at +{latest_spy:.2f}%. Target mean-reversion move on SPY.")
            else:
                st.info("⚖️ **BALANCED / NO DIVERGENCE**: Both indices are moving in tight correlation.")
        elif latest_spy < 0 and latest_dia < 0:
            st.warning("🔻 **BOTH DOWN**: Market is declining from the open. Strategy filters active (Long signals paused).")
        else:
            st.info("👀 **MIXED DIRECTION**: Indices are splitting direction. Waiting for synchronized breakout.")
        ...
        st.warning("Market hasn't opened yet for today (9:30 AM EST). Waiting for session start...")
